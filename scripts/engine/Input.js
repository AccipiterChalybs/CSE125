...
const Input = {
    ...
    init: function(options){
        ...
        // Mouse buttons
        document.body.onmouseup = (e)=> Input._options.axes.filter((axis)=>axis.type===InputType.mouseButton)
                                             .forEach((axis)=>updateAxisInt(axis,e.button, true));
        document.body.onmousedown = (e)=> Input._options.axes.filter((axis)=>axis.type===InputType.mouseButton)
        ...
        // Keyboard buttons
        document.body.onkeyup = (e)=> Input._options.axes.filter((axis)=>axis.type===InputType.keyboard)
                                           .forEach((axis)=>updateAxisInt(axis,e.key, true));
        document.body.onkeydown = (e)=> Input._options.axes.filter((axis)=>axis.type===InputType.keyboard)
                                           .forEach((axis)=>updateAxisInt(axis,e.key));

        // Mouse Movement
        document.body.onmousemove = (e)=> Input._options.axes.filter((axis)=>axis.type===InputType.mouse)
