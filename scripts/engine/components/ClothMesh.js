...
/*
    for (let i = 0; i < mesh.vertices.length/3; ++i) {
      for (let p = 0; p < Mesh.prototype.POSITION_COUNT; ++p) {
        clothArray.push(mesh.vertices[i*3 + p]);
      }
      for (let p = 0; p < Mesh.prototype.NORMAL_COUNT; ++p) {
        clothArray.push(mesh.normals[i*3 + p]);
      }
      if (enabledTexCoord[0]) {
        for (let p = 0; p < Mesh.prototype.TEX_COORD_COUNT; ++p) {
          staticArray.push(mesh.texturecoords[0][i*Mesh.prototype.TEX_COORD_COUNT + p]);
        }
      }
      if (hasTangents) {
        for (let p = 0; p < Mesh.prototype.TANGENT_COUNT; ++p) {
          clothArray.push(mesh.tangents[i*Mesh.prototype.TANGENT_COUNT + p]);
        }
        for (let p = 0; p < Mesh.prototype.TANGENT_COUNT; ++p) {
          clothArray.push(mesh.bitangents[i*Mesh.prototype.TANGENT_COUNT + p]);
        }
      }
      if (hasBones) {
        for (let p = 0; p < Mesh.prototype.BONE_WEIGHT_COUNT; ++p) {
          staticArray.push(boneResults[i].second[p]); //TODO check correctness
        }
    ...
    let stride1 = Mesh.prototype.FLOAT_SIZE * (Mesh.prototype.POSITION_COUNT + Mesh.prototype.NORMAL_COUNT);
    if (hasTangents) stride1 += Mesh.prototype.FLOAT_SIZE * (2 * Mesh.prototype.TANGENT_COUNT);
    let currentOffset = 0;
    GL.vertexAttribPointer(0, 3, GL.FLOAT, false, stride1, currentOffset);
    currentOffset += (Mesh.prototype.FLOAT_SIZE * 3);
    GL.vertexAttribPointer(1, 3, GL.FLOAT, false, stride1, currentOffset);
    ...
    let staticBuffer = GL.createBuffer();
    GL.bindBuffer(GL.ARRAY_BUFFER, staticBuffer);
    GL.bufferData(GL.ARRAY_BUFFER, new Float32Array(staticArray), GL.STREAM_DRAW);


    let stride2 = 0;
    if (enabledTexCoord[0]) stride2 += Mesh.prototype.FLOAT_SIZE * Mesh.prototype.TEX_COORD_COUNT;
    if (hasBones) stride2 += Mesh.prototype.FLOAT_SIZE * Mesh.prototype.BONE_WEIGHT_COUNT;
