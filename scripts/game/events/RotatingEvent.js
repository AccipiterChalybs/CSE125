...
class RotatingEvent extends SingingEvent{
  ...
  start() {
    ...
    this._rot = this.transform.getWorldRotation();
    this._oldcharge = this.currentCharge / this.maximumCharge * this._circum;
  ...
  movePlatform() {
    this.transform.setWorldRotation(this._rot);
    let newRot = this.currentCharge / this.maximumCharge * this._circum;
    this.transform.rotateZ(newRot);

    let deltaY = newRot - this._oldcharge;
